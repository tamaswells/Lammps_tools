...
class XDATCAR(Energy_Temp):
    ...
    def writepdb(self,pdb_frame):
        ...
        if not os.path.exists("batch"):
            os.mkdir("batch")        
        if not os.path.exists("batch/%d" %pdb_frame):
            os.mkdir("batch/%d" %pdb_frame)
        tobewriten.append("%06d" %pdb_frame)
        tobewriten.append('1.0')
        for i in self.lattice:
            #print(i)
            tobewriten.append("%9.3f %9.3f %9.3f" %(i[0],i[1],i[2]))
        tobewriten.append(' '.join(self.element_list))
        tobewriten.append(' '.join(list(map(str,self.element_amount))))
        tobewriten.append('Car')
        for i in range(len(self.total_elements)):
            tobewriten.append('%8.3f%8.3f%8.3f' %(self.cartesian_position[i][0],self.cartesian_position[i][1],self.cartesian_position[i][2]))     
        
        with open("batch/%d/POSCAR" %pdb_frame,'w') as pdbwriter:
            tobewriten=[i+'\n' for i in tobewriten]
...
if __name__ == "__main__":
    ...
    parser.add_option("-p", "--pdb",  
                      action="store_true", dest="format_trans", default=True,  
