...
class Lammps(dump):
    def __init__(self,dump_file):
        super(Lammps,self).__init__(dump_file)
        
    def compute(self,parmas):
        flag=0
        time_vec = []
        atoms_info=[]
        while 1:
            which,time,flag = self.iterator(flag)
            if flag == -1: break 
            time,box,atoms,bonds,tris,lines = self.viz(which)
            time_vec.append(time)
            atoms = sorted(atoms)    
            atoms_info.append(atoms)  
            lx = box[3] - box[0]   
            ly = box[4] - box[1]
            lz = box[5] - box[2]
        # for example 
        types = {1.0:"Si",2.0:"C"}
        self.write_xyz(atoms_info,types)
      
    @staticmethod
    def write_xyz(atoms,types):
        with open("traj.xyz",'w') as writer:
            for i in atoms:
                writer.write("%d\n\n" % len(i))
                for j in i:
